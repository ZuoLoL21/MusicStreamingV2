...
func NewProxyHandler(popularityClient *clients.PopularityClient, logger *zap.Logger) *ProxyHandler {
	return &ProxyHandler{
...
func (h *ProxyHandler) ProxyPopularSongsAllTime(w http.ResponseWriter, r *http.Request) {
	libshandlers.ProxyWithServiceJWT(w, r, h.logger, h.popularityClient.ForwardWithServiceJWT)
}
